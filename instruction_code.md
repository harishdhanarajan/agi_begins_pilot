...
rule 1 : no scaffolding (as of now)
rule 2 : no hardcoding
rule 3 : do not keep modifying the code to get the output that i asked for, instead of rethink and modify it
rule 4 : this is a learner so every piece or word of code written must be intended to make sure our model keeps learning
